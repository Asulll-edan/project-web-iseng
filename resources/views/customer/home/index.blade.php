...
@push('styles')
    <style>
        ...
        .hero-bg {
            position: absolute;
            inset: 0;
            background: linear-gradient(135deg, #1a2e22 0%, #2c3e35 40%, #3d5c47 100%);
            z-index: 0
        }
        ...
        .hero-overlay {
            position: absolute;
            inset: 0;
            background: url("data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cg fill='%238aaa92' fill-opacity='0.04'%3E%3Ccircle cx='30' cy='30' r='2'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E");
            z-index: 1
        }
        ...
        .hero-badge {
            ...
            align-items: center;
            gap: 8px;
            background: rgba(158, 195, 167, 0.15);
            border-radius: 5px;
            padding: 7px 16px;
            font-size: 12px;
            font-weight: 600;
        ...
        .banner-slide-inner .badge-promo {
        ...
        .menu-card:hover {
            ...
            box-shadow: 0 24px 60px rgba(90, 124, 101, .18)
        }
...
@section('content')
                ...
                <div class="hero-content" data-aos="fade-right">
                    ...
                    <div class="hero-actions">
                        <a href="{{ route('menu.index') }}" class="btn-primary"
                            style="background:var(--sage-light)!important">
                            <i class="ti ti-bowl-chopsticks"></i> Lihat Menu
                        </a>
                        <a href="{{ route('reservation.index') }}" class="btn-outline"
                                ...
                                @if (file_exists(public_path('storage/' . $banner->image)))
                                    <a href="{{ $banner->link ?? '#' }}"
                                        ...
                                        <img src="{{ $banner->image_url }}" style="width:100%;height:100%;object-fit:cover"
                                            alt="{{ $banner->title }}">
                                        <div
                                            style="position:absolute;inset:0;background:linear-gradient(90deg,rgba(0,0,0,.5),transparent);display:flex;align-items:center;padding:0 48px">
                                            <div style="color:#fff">
                                                <h3 style="font-size:22px;font-weight:700;margin-bottom:6px">
                                                    {{ $banner->title }}</h3>
                                                <p style="font-size:14px;opacity:.85">{{ $banner->description }}</p>
                                            </div>
                                        </div>
                                    </a>
                                @else
    ...
    <section style="padding:56px 0 32px">
        <div class="container">
            ...
            <div style="display:flex;gap:10px;overflow-x:auto;padding-bottom:8px;scrollbar-width:none" data-aos="fade-up"
                data-aos-delay="100">
                <a href="{{ route('menu.index') }}" class="cat-pill active">🍽️ Semua Menu</a>
                @foreach ($categories as $cat)
                    <a href="{{ route('menu.index', ['category' => $cat->slug]) }}" class="cat-pill">{{ $cat->icon }}
                        {{ $cat->name }}</a>
                @endforeach
            </div>
        </div>
            ...
            <div class="container">
                <div style="display:flex;align-items:flex-end;justify-content:space-between;margin-bottom:28px"
                    ...
                    <a href="{{ route('menu.index') }}"
                        style="font-size:13px;color:var(--sage);font-weight:600;display:flex;align-items:center;gap:4px;white-space:nowrap">
                ...
                <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:20px">
                    @foreach ($bestSeller as $menu)
                        <div class="menu-card" data-aos="fade-up" data-aos-delay="{{ $loop->index * 60 }}">
                            <div
                                style="position:relative;overflow:hidden;height:180px;background:var(--beige);display:flex;align-items:center;justify-content:center">
                                @if ($menu->image)
                                    <img src="{{ $menu->image_url }}" class="menu-card-img" alt="{{ $menu->name }}"
                                        loading="lazy">
                                @else
                                    <span style="font-size:52px">🍱</span>
                                @endif
                                <div style="position:absolute;top:10px;left:10px;display:flex;gap:6px">
                                    <span class="badge badge-gold"><i class="ti ti-flame" style="font-size:11px"></i>Best
                                        Seller</span>
                                </div>
                                @if ($menu->discount_price)
                                    <div
                                        style="position:absolute;top:10px;right:10px;background:#e07a5f;color:#fff;border-radius:8px;padding:3px 8px;font-size:11px;font-weight:700">
                                        DISKON</div>
                                @endif
                            </div>
                            <div class="menu-card-body">
                                ...
                                <div style="display:flex;align-items:center;justify-content:space-between;margin-top:10px">
                                    <div>
                                        @if ($menu->discount_price)
                                            <div
                                                style="font-size:11px;text-decoration:line-through;color:var(--text-muted)">
                                                Rp {{ number_format($menu->price, 0, ',', '.') }}</div>
                                        @endif
                                        <div class="menu-card-price">Rp
                                            {{ number_format($menu->effective_price, 0, ',', '.') }}</div>
                                    </div>
                                    @auth
                                        <button onclick="quickAdd({{ $menu->id }},this)"
                                            style="width:36px;height:36px;background:var(--sage);border:none;border-radius:50%;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all .2s"
                                            title="Tambah ke keranjang">
                                            <i class="ti ti-plus" style="color:#fff;font-size:18px"></i>
                                        </button>
                                    @else
                                        <a href="{{ route('login') }}"
                                            style="width:36px;height:36px;background:var(--sage);border-radius:50%;display:flex;align-items:center;justify-content:center">
                                            <i class="ti ti-plus" style="color:#fff;font-size:18px"></i>
                                        </a>
                                    @endauth
                                </div>
    ...
    <section class="stats-section">
        <div class="container">
            <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:32px">
                @foreach ([['ti-users', 'happy_customers', 'Pelanggan Happy'], ['ti-shopping-bag', 'total_orders', 'Pesanan Selesai'], ['ti-bowl-chopsticks', 'total_menu', 'Menu Tersedia'], ['ti-star', 'rating_avg', 'Rating Rata-rata']] as $s)
                    <div class="stat-counter" data-aos="fade-up" data-aos-delay="{{ $loop->index * 80 }}">
                        <i class="ti {{ $s[0] }}"
                            style="font-size:32px;color:rgba(255,255,255,.4);margin-bottom:10px;display:block"></i>
        ...
        <section style="padding:70px 0">
            <div class="container">
                <div style="display:flex;align-items:flex-end;justify-content:space-between;margin-bottom:32px"
                    ...
                    <a href="{{ route('menu.index') }}"
                        style="font-size:13px;color:var(--sage);font-weight:600;display:flex;align-items:center;gap:4px">
                        Lihat semua <i class="ti ti-arrow-right"></i>
                    </a>
                </div>
                <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:20px">
                    @foreach ($featured->take(4) as $menu)
                        <a href="{{ route('menu.show', $menu->slug) }}" class="menu-card card-hover" data-aos="fade-up"
                            data-aos-delay="{{ $loop->index * 70 }}">
                            <div
                                style="height:180px;background:var(--beige);overflow:hidden;display:flex;align-items:center;justify-content:center">
                                @if ($menu->image)
                            ...
                            <div class="menu-card-body">
                                <div class="menu-card-name">{{ $menu->name }}</div>
                                <div class="menu-card-rating"><i class="ti ti-star-filled"
                                        style="color:#f59e0b;font-size:13px"></i>{{ number_format($menu->rating, 1) }}
                                </div>
                                <div class="menu-card-price" style="margin-top:8px">Rp
                                    {{ number_format($menu->effective_price, 0, ',', '.') }}</div>
                            </div>
                        </a>
                    @endforeach
    ...
    <section style="padding:0 0 80px">
        <div class="container">
            <div data-aos="fade-up"
                style="background:linear-gradient(135deg,var(--sage-dark),var(--sage));border-radius:24px;padding:48px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:24px">
                <div>
                    <div
                ...
                <div style="display:flex;gap:12px;flex-wrap:wrap">
                    <a href="{{ route('menu.index') }}"
                        style="padding:13px 28px;background:#fff;color:var(--sage-dark);border-radius:20px;font-weight:700;font-size:14px;display:inline-flex;align-items:center;gap:8px;transition:all .2s">
                        <i class="ti ti-bowl-chopsticks"></i> Pesan Sekarang
                    </a>
                    @guest
                        <a href="{{ route('register') }}"
                            style="padding:13px 28px;background:rgba(255,255,255,.15);color:#fff;border-radius:20px;font-weight:700;font-size:14px;border:1.5px solid rgba(255,255,255,.3);display:inline-flex;align-items:center;gap:8px;transition:all .2s">
...
@push('scripts')
    <script>
        ...
        function quickAdd(menuId, btn) {
            btn.disabled = true;
            btn.innerHTML =
                '<i class="ti ti-loader" style="color:#fff;font-size:18px;animation:spin 1s linear infinite"></i>';
            fetch('/cart/add', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': CSRF,
                ...
                .then(d => {
                    ...
                    if (badge && d.cart_count) {
                        badge.textContent = d.cart_count;
                        badge.style.display = 'flex';
                    }
                    setTimeout(() => {
                        btn.innerHTML = '<i class="ti ti-plus" style="color:#fff;font-size:18px"></i>';
                    }, 1500);
                })
                .catch(() => {
                    btn.disabled = false;
